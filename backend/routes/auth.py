...
from backend.schemas.auth import UserRegister, UserLogin, TokenResponse, UserUpdateRequest
...
async def user_register(body: UserRegister, request: Request) -> TokenResponse:
    return await register_user(body.email, body.password, body.username, request.app.state.supabase)


@router.post("/login", response_model=TokenResponse)
async def user_login(body: UserLogin, request: Request) -> TokenResponse:
...
async def update_account(body: UserUpdateRequest, request: Request, current_user: dict = Depends(get_current_user)):
    return await update_user(current_user["user_id"], body.current_password, body.email, body.password, body.username, request.app.state.supabase)